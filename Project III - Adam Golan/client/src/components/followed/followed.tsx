...
export class Followed extends Component<FollowedProps> {
    render():JSX.Element {
        return (
            <div className="followed" style={{display: (this.props.vacations.length === 0)? "none": "block"}}>
                <h2>your follows</h2>
                {this.props.vacations.map((v:Vacation)=>{
                    ...
                    <label className="edit">
                        <input type="checkbox" id={JSON.stringify(v.id)} onClick={this.props.toggleFollow.bind(this)} defaultChecked/>
                        <i title="Toggle Follow" className="fa fa-heart" style={{ color: "#ff6b6b" }}></i>
                    </label>
                    <h3>{v.destination}</h3>
                    <div className="image">
